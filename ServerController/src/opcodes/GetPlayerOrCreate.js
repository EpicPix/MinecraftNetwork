...
module.exports = function(websocket, json) {
    if(websocket.checkAuth()) {
        if(json['uuid']) {
            if(json['username']) {
                websocket.respond(json, {ok: true, player: require('../index').getPlayerOrCreate({uuid: json['uuid'], username: json['username'], rank: require('../index').getDefaultRank(), firstLogin: Date.now(), lastLogin: -1})});
            }else {
